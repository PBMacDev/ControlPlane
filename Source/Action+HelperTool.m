...
- (BOOL) helperToolPerformAction: (NSString *) action withParameter: (id) parameter {
    ...
    if (!versionCheck) {
        ...
        NSNumber *version = [(__bridge NSDictionary *) helperToolResponse objectForKey: @kCPHelperToolGetVersionResponse];
...
- (OSStatus) helperToolActualPerform: (NSString *) action
    ...
    *response = NULL;
    ...
    if (error != noErr && error != userCanceledErr) {
        ...
        BASDiagnoseFailure(auth, (__bridge CFStringRef) bundleID);
...
BOOL installHelperToolUsingSMJobBless(void) {
    ...
    NSError* error = nil;
    NSDictionary*    installedHelperJobData     = (NSDictionary*)CFBridgingRelease(SMJobCopyDictionary(kSMDomainSystemLaunchd, (CFStringRef)kPRIVILEGED_HELPER_LABEL));
    ...
    if (installedHelperJobData) {
          ...
          if ( currentVersion == installedVersion )
        {
            ...
            stErr = SecRequirementCreateWithString((__bridge CFStringRef)[NSString stringWithFormat:@"identifier %@ and certificate leaf[subject.CN] = \"%@\"", kPRIVILEGED_HELPER_LABEL, @kSigningCertCommonName], kSecCSDefaultFlags, &requirement );
...
BOOL blessHelperWithLabel(NSString* label, NSError** error) {
    ...
    } else
    {
         ...
         */
        //TODO: to find better solution for casting CFError to NSError with ARC
        CFErrorRef cferror = NULL;
        result = SMJobBless(kSMDomainSystemLaunchd, (__bridge CFStringRef)label, authRef, &cferror);
        if (cferror && error) {
            *error = [NSError errorWithDomain:CFErrorGetDomain(cferror) code:CFErrorGetCode(cferror) userInfo:CFBridgingRelease(CFErrorCopyUserInfo(cferror))];
            CFRelease(cferror);
        }
    }
